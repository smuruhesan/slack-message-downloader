...
4. Scroll up in your Slack chat to load older messages. 
   * *Tip: Keep scrolling up until you see `conversations.history` show up in the Developer Tools window.*
...
6. Click the **Payload** tab, locate the `token` parameter (the text will start with `xoxc-`), **right-click** on the token text, and select **Copy Value**.
...
You only need to edit the first four lines to tell the script what data to fetch. Once updated, your script should look like this fully filled-out example:

```javascript
// --- CONFIGURATION ---
const TOKEN = "xoxc-12345678-98765432-11223344-a1b2c3d4e5f6g7h8"; // Pasted from Step A
const CHANNEL_ID = "C0B3ZDH5E74"; // The Channel ID you found (see below)
const START_DATE = "2026-05-01"; // Format: YYYY-MM-DD
const END_DATE = "2026-05-15";   // Format: YYYY-MM-DD
```

**What these values mean and why we need them:**
* **`TOKEN`:** This acts as your secure, temporary ID badge so Slack knows your browser is authorized to view the messages.
* **`CHANNEL_ID`:** The exact system ID for the chat room you want to extract (see instructions below). 
...
1. In your main Slack window, navigate to the channel you want to extract.
2. Click directly on the **Channel Name** at the very top-left of the chat window.
3. A pop-up window with channel details will appear. Click on the **About** tab.
4. Scroll to the very bottom of this tab.
5. You will see the Channel ID listed (e.g., `Channel ID: C0B3ZDH5E74`). 
6. Click the small "Copy" icon next to it and paste it into your script.
